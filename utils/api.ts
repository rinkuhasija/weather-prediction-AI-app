...
export async function fetchHourly(lat: number, lon: number, date: string) {
    const tz = "auto"
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m,precipitation,windspeed_10m,weathercode&start_date=${date}&end_date=${date}&timezone=${tz}`
